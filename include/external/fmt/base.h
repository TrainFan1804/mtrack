...
// Use the provided definition.
    ...
    // GCC only allows constexpr member functions in non-literal types
    // since 7.2: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=66297.
...
#define FMT_PRAGMA_IMPL(x) _Pragma(#x)
    ...
    // Workaround a _Pragma bug
    // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=59884 and an nvhpc warning:
...
// Use the provided definition.
    ...
    {
        FMT_ASSERT(
            std::is_unsigned<Int>::value || value >= 0, "negative value"
        );
    ...
    using unsigned_char
        = conditional_t<sizeof(Char) == 1, unsigned char, unsigned>;
    ...
    struct is_std_string_like<
        T, void_t<decltype(std::declval<T>()
                               .find_first_of(typename T::value_type(), 0))>>
        : std::is_convertible<
...
using basic_format_context = conditional_t<
    std::is_same<OutputIt, appender>::value, context,
    generic_context<OutputIt, Char>>;
using format_context = context;

template <typename Char>
using buffered_context = conditional_t<
    std::is_same<Char, char>::value, context,
...
public:
    ...
    constexpr auto fill_unit() const -> Char
    {
        ...
        return static_cast<Char>(
            static_cast<uchar>(fill_data_[0])
...
public:
    ...
    constexpr explicit parse_context(
        basic_string_view<Char> fmt, int next_arg_id = 0
    )
        : fmt_(fmt),
...
namespace detail
{
    ...
    /// String's character (code unit) type. detail:: is intentional to prevent
    /// ADL.
    template <
        typename S,
...
#define FMT_TYPE_CONSTANT(Type, constant)              \
    ...
    // named_args is non-const to suppress a bogus -Wmaybe-uninitalized in
    // gcc 13.
    template <typename Char>
    ...
    template <typename Char, typename T, FMT_ENABLE_IF(is_named_arg<T>::value)>
    void init_named_arg(
        named_arg_info<Char> *named_args, int &arg_index, int &named_arg_index,
        const T &arg
    )
    {
    ...
    template <
        typename T, typename Char, FMT_ENABLE_IF(is_static_named_arg<T>::value)>
    FMT_CONSTEXPR void init_static_named_arg(
        named_arg_info<Char> *named_args, int &arg_index, int &named_arg_index
    )
    {
    ...
    template <typename T>
    using format_as_member_result = remove_cvref_t<
        decltype(formatter<T>::format_as(std::declval<const T &>()))>;
    ...
    {
        return decltype(has_formatter_impl<Char>(static_cast<T *>(nullptr))
        )::value;
    }

    // Maps formatting argument types to natively supported types or
    // user-defined types with formatters. Returns void on errors to be
    // SFINAE-friendly.
    template <typename Char>
    struct type_mapper
    {
        ...
        static auto map(T) -> conditional_t<
            std::is_same<T, char>::value || std::is_same<T, Char>::value, Char,
            void>;
        ...
        template <
            typename T, typename C = char_t<T>,
            FMT_ENABLE_IF(!std::is_pointer<T>::value)>
        static auto map(const T &) -> conditional_t<
            std::is_same<C, Char>::value, basic_string_view<C>, void>;
        ...
        static auto map(const T &x)
            -> decltype(map(formatter<T>::format_as(x)));

        template <typename T, FMT_ENABLE_IF(use_formatter<T>::value)>
        static auto map(T &)
            -> conditional_t<has_formatter<T, Char>(), T &, void>;
    ...
    template <typename T, typename Char>
    using mapped_t
        = decltype(detail::type_mapper<Char>::map(std::declval<T &>()));
    ...
    using stored_type_constant = std::integral_constant<
        type, Context::builtin_types || TYPE == type::int_type
                  ? TYPE
                  : type::custom_type>;
    ...
    public:
        ...
        FMT_CONSTEXPR void check_dynamic_spec(int arg_id)
        {
            ...
            if (arg_id < num_args_ && types_
                && !is_integral_type(types_[arg_id]))
    ...
    FMT_CONSTEXPR auto code_point_length(const Char *begin) -> int
    {
        ...
        return static_cast<int>((0x3a55000000000000ull >> (2 * (c >> 3))) & 3)
               + 1;
    }
    ...
    FMT_CONSTEXPR auto parse_nonnegative_int(
        const Char *&begin, const Char *end, int error_value
    ) noexcept -> int
    {
    ...
    FMT_CONSTEXPR auto
    parse_arg_id(const Char *begin, const Char *end, Handler &&handler)
        -> const Char *
    {
        Char c = *begin;
        if (c >= '0' && c <= '9')
    ...
    FMT_CONSTEXPR auto parse_width(
        ...
        arg_ref<Char> &width_ref, parse_context<Char> &ctx
    ) -> const Char *
    {
        auto result
            = parse_dynamic_spec(begin, end, specs.width, width_ref, ctx);
    ...
    ) -> const Char *
    {
        ...
        auto result = parse_dynamic_spec(
            begin, end, specs.precision, precision_ref, ctx
        );
    ...
    ) -> const Char *
    {
        ...
        struct
        {
            ...
            FMT_CONSTEXPR auto operator()(pres pres_type, int set)
                -> const Char *
            {
                if (!in(arg_type, set))
                    report_error("invalid format specifier");
    ...
    case '6': case '7': case '8': case '9': case '{':
            ...
            case '.':
                ...
                begin = parse_precision(
                    begin, end, specs, specs.precision_ref, ctx
                );
                break;
            case 'L':
    ...
    // Checks char specs and returns true iff the presentation type is
    // char-like.
    FMT_CONSTEXPR inline auto check_char_specs(const format_specs &specs)
        -> bool
    {
    ...
    template <
        typename Char, int NUM_ARGS, int NUM_NAMED_ARGS, bool DYNAMIC_NAMES>
    ...
    public:
              ...
              parse_funcs_ {&invoke_parse<T, Char>...}
        {
            ...
            FMT_APPLY_VARIADIC(init_static_named_arg<T>(
                named_args_, arg_index, named_arg_index
            ));
            ...
            -> const Char *
        {
            ...
            // If id is out of range, it means we do not know the type and
            // cannot parse the format at compile time. Instead, skip over
            // content until we finish the format spec, accounting for any
            // nested replacements.
    ...
    protected:
        ...
        constexpr buffer(
            grow_fun grow, T *p = nullptr, size_t sz = 0, size_t cap = 0
        ) noexcept
            : ptr_(p),
              size_(sz),
              capacity_(cap),
...
// template 'void fmt::v11::detail::buffer<T>::append(const U *,const U *)'.
    ...
    struct has_back_insert_iterator_container_append<
        ...
        void_t<decltype(get_container(std::declval<OutputIt>())
                            .append(
                                std::declval<InputIt>(), std::declval<InputIt>()
                            ))>> : std::true_type
    {
    };
    ...
    struct custom_value
    {
        ...
        void *value;
        void (*format)(
            void *arg, parse_context<char_type> &parse_ctx, Context &ctx
        );
    };
    ...
    public:
        ...
        value(const T &)
        {
            ...
            // format a pointer cast it to `void*` or `const void*`. In
            // particular, this forbids formatting of `[const] volatile char*`
            // printed as bool by iostreams.
            static_assert(
                sizeof(T) == 0, "formatting of non-void pointers is disallowed"
            );
        }
    ...
    private:
        ...
        FMT_CONSTEXPR value(T &x, custom_tag)
        {
            ...
            // T may overload operator& e.g. std::vector<bool>::reference in
            // libc++.
            if (!is_constant_evaluated())
            {
            ...
            else
            {
                ...
                if constexpr (std::is_same<
                                  decltype(&x), remove_reference_t<T> *>::value)
        ...
        static void format_custom(
            void *arg, parse_context<char_type> &parse_ctx, Context &ctx
        )
        {
            auto f = Formatter();
            parse_ctx.advance_to(f.parse(parse_ctx));
            using qualified_type = conditional_t<
                has_formatter<const T, char_type>(), const T, T>;
    ...
    struct is_output_iterator<
        ...
        enable_if_t<std::is_assignable<
            decltype(*std::declval<decay_t<It> &>()++), T>::value>>
        : std::true_type
    {
    };
    ...
    using arg_t = conditional_t<
        NUM_ARGS <= max_packed_args, value<Context>, basic_format_arg<Context>>;

    template <
        typename Context, int NUM_ARGS, int NUM_NAMED_ARGS,
        unsigned long long DESC>
    struct named_arg_store
    {
        // args_[0].named_args points to named_args to avoid bloating
        // format_args.
    ...
    // such as `vformat`. It is a plain struct to reduce binary size in debug
    // mode.
    template <
        typename Context, int NUM_ARGS, int NUM_NAMED_ARGS,
        unsigned long long DESC>
    struct format_arg_store
    {
        ...
        using type = conditional_t<
            NUM_NAMED_ARGS == 0, arg_t<Context, NUM_ARGS>[max_of(1, NUM_ARGS)],
    ...
    FMT_API void vformat_to(
        buffer<char> &buf, string_view fmt, format_args args,
        locale_ref loc = {}
    );
...
public:
    ...
    auto format_custom(
        const char_type *parse_begin, parse_context<char_type> &parse_ctx,
        Context &ctx
    ) -> bool
    {
...
public:
    ...
    template <
        int NUM_ARGS, int NUM_NAMED_ARGS, unsigned long long DESC,
    ...
    /// arguments.
    constexpr basic_format_args(
        const format_arg *args, int count, bool has_named = false
    )
        : desc_(
...
public:
        ...
        : str(s, N - 1)
    {
        ...
        static_assert(
            count<(is_view<remove_cvref_t<T>>::value
        ...
        static_assert(
            ...
            "FMT_ENFORCE_COMPILE_STRING requires format strings to use "
            "FMT_STRING"
        );
#endif
    }
...
template <
    typename Context = context, typename... T, int NUM_ARGS = sizeof...(T),
...
template <typename... T>
using vargs = detail::format_arg_store<
    context, sizeof...(T), detail::count_named_args<T...>(),
...
template <
    typename OutputIt, FMT_ENABLE_IF(detail::is_output_iterator<
